...
public static class LanguageValidation {
  public static void Validate(IReadOnlyList<HixIrNode> declarations, List<HixParseDiagnostic> diagnostics, HixBackend backend = null) {
    backend ??= HixCoreBackend.Instance;
    void Error(HixIrNode node, string message) => diagnostics.Add(new HixParseDiagnostic(node.Line, message));
    ...
    void Scope(IReadOnlyList<HixIrNode> nodes, IReadOnlyList<FunctionDeclarationIr> imported) {
      var functions = nodes.OfType<FunctionDeclarationIr>().ToArray();
      ...
      void Pure(BlockStatementIr body) {
        foreach (var node in Descendants(body)) {
          if (node is AssignmentStatementIr {Storage: not StorageSpace.Local} or AssignmentStatementIr {IsCarried: true})
            Error(node, "pure functions cannot mutate shared storage");
          if (node is RootExpressionIr {IsSmart: false} root && (root.Name is "var" or "tar" || backend.Roots.TryGetValue(root.Name, out var hostRoot) && hostRoot.HasEffects))
            Error(node, "pure functions cannot read host or shared storage");
          if (node is CallExpressionIr call && !visible.Any(candidate => candidate.Name == call.Name) &&
      ...
      foreach (var lambda in nodes.SelectMany(Descendants).OfType<LambdaExpressionIr>()) Pure(lambda.Body);
      foreach (var function in nodes.OfType<FunctionDeclarationIr>())
        foreach (var assignment in Descendants(function.Body).OfType<AssignmentStatementIr>().Where(item => item.IsCarried))
          Error(assignment, "carry local is only valid in top-level prelude expressions");
      foreach (var lambda in nodes.SelectMany(Descendants).OfType<LambdaExpressionIr>())
        foreach (var assignment in Descendants(lambda.Body).OfType<AssignmentStatementIr>().Where(item => item.IsCarried))
          Error(assignment, "carry local is only valid in top-level prelude expressions");
      foreach (var expression in nodes.OfType<ExpressionDeclarationIr>().Where(item => !item.IsPrelude))
        foreach (var assignment in Descendants(expression.Body).OfType<AssignmentStatementIr>().Where(item => item.IsCarried))
      ...
      foreach (var boundary in nodes.SelectMany(node => node switch {
        FunctionDeclarationIr function => new[] {function.Body},
        ExpressionDeclarationIr expression => new[] {expression.Body},
        ...
        foreach (var node in Descendants(boundary)) {
          var label = node switch {
            ...
            ControlFlowStatementIr {Operation: ControlFlowKind.Label} marker => marker.Label, _ => null
          };
    ...
    foreach (var group in declarations.OfType<MixinDeclarationIr>().GroupBy(mixin => mixin.Name, StringComparer.Ordinal))
