...
public sealed class HixProgramImage {
  ...
  public HixProgramImage(
    ...
    ConstantPool = Array.AsReadOnly((IHixValue[])constants.Clone());
    StringPool = strings;
  ...
  private static void AppendScopeIdentity(HixFingerprintBuilder builder, LanguageFunctionScope scope) {
  ...
  private static void AppendFieldsIdentity(HixFingerprintBuilder builder, IReadOnlyList<BytecodeField> fields) {
  ...
  public void Fingerprint(HixFingerprintBuilder builder) {
    ...
    foreach (var value in ConstantPool) {
      ...
      if (value is NumberHixValue number) builder.Append(number.Value);
