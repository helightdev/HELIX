...
class HixDocumentationProvider : DocumentationProvider {
    ...
    private fun documentation(element: PsiElement, originalElement: PsiElement?): String? {
        ...
        if (fact != null && fact.documentation.isNotBlank()) return render(fact.type, fact.documentation)
        ...
        val definitions = service.definitions.filter { it.name == context.text &&
        ...
        return fact?.let { render(it.type, it.documentation) }
    }
