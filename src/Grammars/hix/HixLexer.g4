...
tokens {
 ...
 FAT_ARROW, NUMBER, BOOLEAN, NULL, MISSING, QUESTION, PIPE
}
...
SEMICOLON: ';';
COMMA: ',';
TOPLEVEL_QUESTION: '?' -> type(QUESTION);
TOPLEVEL_PIPE: '|' -> type(PIPE);
...
TOPLEVEL_DERIVATION: Identifier ':' -> pushMode(TOPLEVEL_DERIVATION_MODE);
TOPLEVEL_SMART_ROOT: '$' -> type(VALUE_SMART_ROOT);
...
TOPLEVEL_METADATA: '%' -> pushMode(METADATA_MODE), type(METADATA_PREFIX);
...
METADATA_SEMICOLON: ';' -> popMode, type(SEMICOLON);
...
mode TOPLEVEL_DERIVATION_MODE;
DERIVATION_PREDICATE: ':?' -> pushMode(FUNCTION_IDENTIFIER_MODE), type(VALUE_PREDICATE);
DERIVATION_FUNCTION: ':' -> pushMode(FUNCTION_IDENTIFIER_MODE), type(VALUE_FUNCTION);
DERIVATION_MEMBER: '#' -> pushMode(MEMBER_IDENTIFIER_MODE), type(VALUE_MEMBER);
DERIVATION_BEGIN_ARGUMENT: '<' -> pushMode(ARGUMENT_MODE), type(BEGIN_ARGUMENT);
DERIVATION_BEGIN_PARAMETERS: '(' -> pushMode(VALUE_MODE), type(BEGIN_PARAMETERS);
DERIVATION_CHECK: '?' -> type(QUESTION);
DERIVATION_IDENTIFIER: Identifier -> type(FUNCTION_IDENTIFIER);
DERIVATION_SEMICOLON: ';' -> popMode, type(SEMICOLON);
DERIVATION_WHITESPACE: Whitespace -> popMode, channel(HIDDEN);
DERIVATION_NEWLINE: Newline -> popMode, type(NEWLINE);
INVALID_DERIVATION: . -> popMode, type(ERROR_TOKEN);
...
mode VALUE_MODE; //
VALUE_DELEGATE: 'delegate' -> type(KEYWORD_DELEGATE);
...
VALUE_MISSING: 'missing' -> type(MISSING);
...
NOT_VALUE: '!';
VALUE_CHECK: '?' -> type(QUESTION);
VALUE_PIPE: '|' -> type(PIPE);
