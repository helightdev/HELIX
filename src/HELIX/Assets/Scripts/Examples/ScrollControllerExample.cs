...
namespace Examples {
  ...
  public class ScrollControllerExampleState : State<ScrollControllerExample> {
    ...
    public override Widget Build(BuildContext context) {
      return new HColumn(gap: 16, crossAxisAlign: Align.Stretch, modifiers: new Modifier[] { MarginModifier.Of(16) }) {
        ...
        new HRow(gap: 16f) {
          new HBox(borderRadius: 8) {
            new HScrollView(controller: _controller) {
              new HBox(background: MaterialColors.Red.Value).Size(height: 280, width: StyleKeyword.Auto),
              new HBox(background: MaterialColors.Blue.Value).Size(height: 480, width: StyleKeyword.Auto),
              new HBox(background: MaterialColors.Green.Value).Size(height: 640, width: StyleKeyword.Auto),
              new HBox(background: MaterialColors.Purple.Value).Size(height: 360, width: StyleKeyword.Auto)
            }
