...
namespace DefaultNamespace {
                ...
                children = new Widget[] {
                    new BoxShadow {
                        child = new Container {
                            ...
                            child = isOn ? new UnityLabel("Hello :3") : null,
                            Modifiers = new Modifier[] {
                                TransitionModifier.Of(new Transition(StyleProperties.BackgroundColor) { duration = 1f })
                            }
                        }
                    },
                    new UnityLabel($"Hello World! {counter}"), //
                    new TestComponent(), new ButtonBuilder {
                        enabled = isOn,
                        selected = counter % 5 == 0,
                        builder = (_, state) => {
                            if (state.Pressed()) return new FlexRow { children = boxes };
                            return new UnityLabel(state.ToStateString());
                        }
                    },
                    new SimpleButton {
                        style = buttonStyle,
                        child = new UnityLabel("Toggle"),
                        selected = isOn,
                        onClick = SetState(() => {
                                isOn = !isOn;
                                counter++;
    ...
    public class TestComponent : StatelessWidget<TestComponent> {
        public override Widget Build(BuildContext context) {
            return new UnityLabel("Hello from TestComponent!");
        }
    }
}