...
namespace HELIX.Examples {
  ...
  public partial class NWSystemExample : VisualElement {
    ...
    private readonly struct ViewBindings {
      public readonly Composable<OverlayController> overlayBody;
      public readonly Composable<FormController> formBody;
      public readonly Composable<int> formListItem;
      public readonly Composable modalBody;
      public readonly OverlayComposable modalContent;
      ...
      public ViewBindings(NWSystemExample owner) {
        overlayBody = owner.ComposeApplication;
        formBody = owner.ComposeForm;
        formListItem = owner.ComposeFormListItem;
        modalBody = owner.ComposeModalBody;
        modalContent = owner.ComposeModal;
    ...
    private void ComposeRoot(ref Composition cx) {
      ...
      cx.OverlayHost(_view.overlayBody, _overlays);
    }
    ...
    private void ComposeForm(ref Composition cx, FormController form) {
      using (cx.Flex(Axis.Vertical, cross: Align.Stretch)) {
        cx.FormTextInput(
          ...
          decorator: new FormFieldDecoratorSpec(label: new LabelSpec("Display name"))
        );
        ...
        cx.FormIntInput(
          ...
          decorator: new FormFieldDecoratorSpec(label: new LabelSpec("Age"))
        );
        cx.FormDropdown(
          "preferences.mode",
          ExampleMode.Balanced,
          _modeOptions,
          decorator: new FormFieldDecoratorSpec(label: new LabelSpec("Mode"))
        );
        cx.FormSlider(
          "preferences.volume",
          0.65f,
          options: _volumeOptions,
          decorator: new FormFieldDecoratorSpec(label: new LabelSpec("Volume"))
        );
        cx.FormCheckbox(
          "preferences.enabled",
          true,
          decorator: new FormFieldDecoratorSpec(
        ...
        cx.FormList("items", 2, _view.formListItem);
        using (cx.Flex(Axis.Horizontal, cross: Align.Center)) {
    ...
    private void ComposeFormListItem(ref Composition cx, int index) {
      using (cx.Flex(Axis.Horizontal, cross: Align.Center)) {
        cx.FormTextInput(
          "label",
          "Item",
          decorator: new FormFieldDecoratorSpec(label: new LabelSpec("Label"))
        );
        cx.FormIntInput(
          "amount",
          1,
          decorator: new FormFieldDecoratorSpec(label: new LabelSpec("Amount"))
        );
      }
    }
    ...
    private void ComposeModal(ref Composition cx, OverlayHandle handle) {
      cx.OverlayPanel(_view.modalBody);
    }
    ...
    private void OpenModal(IBoundary boundary) {
      ...
      _modal = _overlays.ShowModal(_view.modalContent, dismissed: _view.modalDismissed);
    }
