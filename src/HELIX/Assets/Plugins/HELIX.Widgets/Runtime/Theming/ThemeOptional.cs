...
namespace HELIX.Widgets.Theming {
    ...
    public class ThemeOptional<T> : ThemeOptional {
        ...
        public static implicit operator ThemeOptional<T>(T value) => new() {
            hasValue = true,
            value = value
        };
        
        public static ThemeOptional<T> None => new() {
            hasValue = false,
            value = default
        };
        ...
        public static implicit operator T(ThemeOptional<T> optional) => optional.hasValue ? optional.value : default(T);

        public static implicit operator ThemeOverride<T>(ThemeOptional<T> optional) => optional.hasValue
            ? new ThemeOverride<T> {
