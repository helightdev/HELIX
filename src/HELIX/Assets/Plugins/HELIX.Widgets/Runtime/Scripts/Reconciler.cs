...
namespace HELIX.Widgets {
  public static class Reconciler {
    ...
    private static bool _collectionLock;
    ...
    public static void ReconcileCollection(
      ...
      if (_collectionLock) {
        Debug.LogError(
          $"Sensitive collection reconciliation is already in progress for another collection. " +
          $"Since scratch buffers are shared and single not thread safe, this is not allowed. " +
      ...
      try {
        ...
        isDirty = descriptors.Count != _existingScratch.Count;
      ...
      if (isDirty) {
        ...
        try {
          var resultCount = _resultScratch.Count;
          var deltaCount = _deltaScratch.Count;
          var resultArray = ArrayPool<IWidgetElement>.Shared.Rent(resultCount);
          var deltaArray = ArrayPool<ReconcilerCollectionDelta>.Shared.Rent(deltaCount);
          _resultScratch.CopyTo(resultArray);
          _deltaScratch.CopyTo(deltaArray);

          try {
            for (var i = 0; i < deltaCount; i++) {
              var current = deltaArray[i];
              if (!current.added) current.target.CallUnmounted();
            }

            for (var i = 0; i < deltaCount; i++) {
              var current = deltaArray[i];
              // Here new reconcile calls may happen, that's why we lease the array above. Initially, I had this also
              // use the scratch buffers, but that prevented me from properly doing "dedicated off-panel" rebuilds.
              if (current.added) current.target.CallMounted(current.descriptor, owner);
            }

            collection.UpdateWidgetElements(resultArray.AsSpan(0, resultCount), deltaArray.AsSpan(0, deltaCount));
          } finally {
            ArrayPool<IWidgetElement>.Shared.Return(resultArray);
            ArrayPool<ReconcilerCollectionDelta>.Shared.Return(deltaArray);
          }
        } catch (Exception ex) {
          Debug.LogError(
    ...
    public static void Reconcile(IWidgetElement element, Widget descriptor) {
      ...
      } finally { BuildContext.ReconcilerCurrent = previous; }
    }
    ...
    private static void CallMounted(this IWidgetElement element, Widget descriptor, IWidgetElement parent) {
      ...
      if (element is IScheduledReconcileRunner runner) runner.TryRunScheduledReconcile(ReconcileMode.AfterParent);
    }
  }
  ...
  public enum ReconcileMode { AfterParent = 0, Eager = -1, AfterAttach = 1 }
  ...
  public readonly struct HierarchyDescriptionCollection : IWidgetElementCollection {
    ...
    public void UpdateWidgetElements(Span<IWidgetElement> result, Span<ReconcilerCollectionDelta> deltas) {
      ...
      for (var i = 0; i < result.Length; i++) {
        var targetElement = result[i].Element;
