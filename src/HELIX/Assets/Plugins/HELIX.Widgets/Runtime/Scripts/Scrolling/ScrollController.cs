...
namespace HELIX.Widgets.Scrolling {
    public class ScrollController : ValueSignal<float>, ISignalObserver {
        ...
        public float Offset {
            ...
            set {
                if (ScrollPosition == null) _lastOffset = value;
                else ScrollPosition.Extent = value;
            }
        }
        ...
        public void AnimateTo(float offset, TimeValue duration, EasingMode easing = EasingMode.Linear) {
            if (ScrollPosition == null) Offset = offset;
            else ScrollPosition.AnimateTo(offset, duration, easing);
        }
        ...
        public void Attach(ScrollPosition position) {
            if (ScrollPosition != null) {
                HelixDiagnostics.Build(
                    ...
                    "This error occurs when trying to attach a ScrollController to a ScrollPosition " +
            ...
            if (KeepScrollOffset) position.Restore(_lastOffset.GetValueOrDefault(position.Extent));
            else position.Restore(_initialScrollOffset);
        }
        ...
        public void Detach(ScrollPosition position) {
            if (ScrollPosition != position) {
                HelixDiagnostics.Build(
                    ...
                    "This error occurs when trying to detach a ScrollController from a ScrollPosition " +
