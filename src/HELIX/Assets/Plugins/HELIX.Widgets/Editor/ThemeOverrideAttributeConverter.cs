...
namespace HELIX.Widgets.Editor {
    public static class PolymorphicUxmlConverter {
        ...
        public static bool TryConvertFromString(Type type, string value, out object result) {
            Load();
            var actualType = type;
            if (type == typeof(Texture2D) || type == typeof(Sprite) || type == typeof(VectorImage)) {
                actualType = typeof(Background);
            }
            ...
            switch (result) {
                case Background background when type == typeof(Texture2D):
                    result = background.texture;
                    break;
                case Background background when type == typeof(Sprite):
                    result = background.sprite;
                    break;
                case Background background  when type == typeof(VectorImage):
                    result = background.vectorImage;
                    break;
            }
            return success;
        }
        ...
        public override ThemeOverride<T> FromString(string value) {
            try {
                ...
                if (value.StartsWith("value:")) {
                    ...
                    if (!success) {
                        throw new InvalidOperationException(
                            $"Cannot convert constant value string to type {typeof(T)}.");
                    }
