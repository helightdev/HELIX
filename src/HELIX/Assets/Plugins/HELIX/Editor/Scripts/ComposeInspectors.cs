...
namespace HELIX.Editor {
  ...
  public sealed class InspectorTreeNode {
    ...
    public InspectorTreeNode(
      ...
      this.columns = columns?.ToDictionary(entry => entry.Key, entry => entry.Value) ?? new Dictionary<string, string>();
  ...
  public abstract class HelixHierarchyInspectorWindow : EditorWindow {
    public sealed class InspectorNodeTypeHandler : HierarchyNodeTypeHandler {
      ...
      public bool SetParent(in HierarchyNode node, in HierarchyNode parent, int index) => CommandList.SetParent(in node, in parent, index);
    ...
    public void CreateGUI() {
      ...
      if (SupportsLiveRefresh) {
        _liveToggle = new ToolbarToggle { text = "Live", value = true };
      ...
      var split = new TwoPaneSplitView(1, 500, TwoPaneSplitViewOrientation.Horizontal);
      split.style.flexGrow = 1;
      rootVisualElement.Add(split);

      var hierarchyPane = new VisualElement().WithStyle(style => style.flexGrow = 1);
      _summaryLabel = new Label();
      _summaryLabel.style.paddingLeft = 6;
      _summaryLabel.style.paddingTop = 3;
      _summaryLabel.style.paddingBottom = 3;
      hierarchyPane.Add(_summaryLabel);
      ...
      _hierarchyView.style.flexGrow = 1;
      _hierarchyView.SetSourceHierarchy(_hierarchy, HierarchyNodeFlags.None);
      ...
      RefreshFromSource();
      if (SupportsLiveRefresh) _scheduledRefresh = rootVisualElement.schedule.Execute(RefreshFromSource).Every(500);
    }
    ...
    protected static string SafeToString(object value) {
      ...
      try { return value.ToString(); } catch (Exception exception) { return $"<ToString threw {exception.GetType().Name}>"; }
    }
  }
}
