...
namespace HELIX.Coloring {
  ...
  public sealed class RadixSwatch {
    ...
    public static implicit operator Color(RadixSwatch swatch) {
      return swatch.W500;
    }

    public static implicit operator StyleColor(RadixSwatch swatch) {
      return swatch.W500;
    }

    public OkLchColor[] GetLch(Brightness brightness) {
      return brightness == Brightness.Dark ? darkLch : lightLch;
    }

    public Color[] GetColors(Brightness brightness) {
      return brightness == Brightness.Dark ? darkColors : lightColors;
    }

    public RadixPalette GetPalette(Brightness brightness) {
