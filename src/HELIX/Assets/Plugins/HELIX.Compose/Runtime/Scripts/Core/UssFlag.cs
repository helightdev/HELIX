...
namespace HELIX.Compose {
  ...
  public static class UssDirtyFlagsExtensions {
    ...
    private static void ClearFlagsFast(this UssFlag flags, VisualElement element) {
      if (flags.HasFlag(UssFlag.Name)) element.name = null;
      if (flags.HasFlag(UssFlag.Classes)) element.ClearClassList();
      ...
      if ((flags & UssFlag.MaskUssStyle) != 0) element.style.Clear();
    }
