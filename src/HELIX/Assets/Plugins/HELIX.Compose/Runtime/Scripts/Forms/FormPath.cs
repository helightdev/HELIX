...
namespace HELIX.Compose {
  ...
  public readonly struct FormPath : IEquatable<FormPath> {
    ...
    public bool Equals(FormPath other) {
      return id == other.id;
    }

    public override bool Equals(object obj) {
      return obj is FormPath other && Equals(other);
    }
    ...
    public static bool operator ==(FormPath left, FormPath right) {
  ...
  internal struct FormPathPart : IEquatable<FormPathPart> {
    ...
    public bool Equals(FormPathPart other) {
      return previous == other.previous && current == other.current && kind == other.kind;
    }

    public override int GetHashCode() {
      return unchecked((previous * 397) ^ current ^ (int)kind);
    }
  }
  ...
  public sealed class FormPathPool {
    ...
    public FormPath Parent(FormPath path) {
      return path.id == 0 ? Root : new FormPath(_parts[path.id].previous);
    }
