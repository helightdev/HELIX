...
namespace HELIX.Compose {
  ...
  public partial class HXFormField : IFormField {
    ...
    public T Metadata<T>() {
      return props.metadata is T value ? value : default;
    }

    public T GetValue<T>(T fallback = default) {
      return Controller == null ? fallback : Controller.GetValue(Path, fallback);
    }

    public void SetUserValue<T>(T value) {
      if (!_registered) throw new InvalidOperationException("Form field is not attached.");
