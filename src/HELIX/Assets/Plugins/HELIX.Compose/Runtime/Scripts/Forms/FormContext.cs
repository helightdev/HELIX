...
namespace HELIX.Compose {
  ...
  public readonly struct FormContext {
    ...
    public FormPath Resolve(string path) {
      return controller.Paths.ParseRelative(prefix, path);
    }

    public FormPath Resolve(int index) {
      return controller.Paths.Append(prefix, index);
    }

    public FormContext WithPrefix(string segment) {
      return new FormContext(controller, Resolve(segment));
    }

    public FormContext WithPrefix(int index) {
      return new FormContext(controller, Resolve(index));
    }
  }

  public static class FormComposition {
    public static bool TryGetForm(this in Composition cx, out FormContext form) {
      return cx.TryReadContext(FormContext.Key, out form);
    }
  ...
  public sealed class FormFieldRegistration<T> : IFormField, IDisposable {
    ...
    public void Dispose() {
      if (!_attached) return;
      _controller.UnregisterField(Path, this);
      _attached = false;
      _controller = null;
    }

    public void OnFormFieldChanged(FormController form, FormPath path) {
      if (!_attached || !ReferenceEquals(form, _controller) || path != Path) return;
      _apply(_controller.GetValue(Path, default(T)), _controller.FieldHasValue(Path));
    }
    ...
    public void Attach(
      ...
      if (_attached && (!ReferenceEquals(_controller, context.controller) || Path != path))
        _controller.UnregisterField(Path, this, false);
    ...
    public void SetUserValue(T value) {
      ...
      _controller.SetValue(Path, value, FormChangeReason.User);
    }
  }
}