...
namespace HELIX.Compose.Forms {
  ...
  public sealed class FormController : Signal {
    public static readonly object NoInitialValue = new object();
    ...
    private bool _pendingNotify, _submitAttempted;
    ...
    public FormPath Path(string path) => Paths.Parse(path);
    public string Format(FormPath path) => Paths.Format(path);
    ...
    public object GetValue(FormPath path, object fallback = null) =>
      _data.TryGetValue(path, out var value) ? value : fallback;

    public T GetValue<T>(string path, T fallback = default) => GetValue(Path(path), fallback);
    public FieldData GetFieldData(FormPath path) => _fields.TryGetValue(path, out var data) ? data : null;
    public FieldData GetFieldData(string path) => GetFieldData(Path(path));
    public bool IsFieldDirty(FormPath path) => GetFieldData(path)?.HasFlag(FieldFlags.Dirty) == true;
    public bool IsFieldTouched(FormPath path) => GetFieldData(path)?.HasFlag(FieldFlags.Touched) == true;
    public bool IsFieldValid(FormPath path) => GetFieldData(path)?.HasFlag(FieldFlags.Error) != true;
    ...
    public void RegisterField(
      FormPath path, IFormField field, IEnumerable<IFormValidator> validators = null,
      ValidationMode validationMode = ValidationMode.OnChange, object initialValue = null,
    ...
    public void RegisterField(
      string path, IFormField field, IEnumerable<IFormValidator> validators = null,
      ValidationMode validationMode = ValidationMode.OnChange, object initialValue = null,
      IEqualityComparer<object> comparer = null, bool enabled = true
    ...
    public void RegisterListField(
      FormPath path, IFormField field, IEnumerable<IFormValidator> validators = null,
      ValidationMode validationMode = ValidationMode.OnSubmit, IEqualityComparer<object> comparer = null,
      bool enabled = true
    ) {
    ...
    public void SetValues(
      IEnumerable<KeyValuePair<FormPath, object>> values, FormChangeReason reason = FormChangeReason.Programmatic
    ) {
    ...
    public bool ValidateField(FormPath path) => ValidateField(path, true);
    ...
    public bool ValidateAll() => ValidatePath(Paths.Root);
    ...
    public IReadOnlyDictionary<FormPath, object> SnapshotData() => new Dictionary<FormPath, object>(_data);

    /// <summary>Formats the current flat path/value data for diagnostics and submit logging.</summary>
    public string FormatData() => FormatData(_data);
    ...
    public int GetListCount(FormPath path) => _fields.TryGetValue(path, out var field) && field.isListField
    ...
    private bool ShouldValidate(FieldData data) => !Skip(data) &&
                                                   ((data.validationMode & ValidationMode.OnChange) != 0 ||
                                                    (data.validationMode & ValidationMode.OnDirty) != 0 &&
                                                    data.HasFlag(FieldFlags.Dirty));

    private static bool Skip(FieldData data) => data.HasFlag(FieldFlags.Stale) || data.HasFlag(FieldFlags.Disabled);
    ...
    private FormPath Parent(FormPath path) => Paths.Parent(path);
    ...
    private void RemapList(FormPath path, Func<int, int> remap) {
      var values = new List<KeyValuePair<FormPath, object>>();
      foreach (var pair in _data)
        if (Paths.TryGetDirectIndex(path, pair.Key, out var index)) values.Add(pair);
      ...
      var fields = new List<KeyValuePair<FormPath, FieldData>>();
      foreach (var pair in _fields)
        if (Paths.TryGetDirectIndex(path, pair.Key, out var index)) fields.Add(pair);
      foreach (var pair in fields) _fields.Remove(pair.Key);
      foreach (var pair in fields) _fields[ReplaceIndex(path, pair.Key, remap(DirectIndex(path, pair.Key)))] = pair.Value;
    }
    ...
    private FormPath ReplaceIndex(FormPath list, FormPath path, int index) =>
      Paths.ReplaceDirectIndex(list, path, index);
    ...
    private void NotifyField(FormPath path, FieldData data) => data.field?.OnFormFieldChanged(this, path);
    private static void AppendFormattedValue(StringBuilder result, object value) {
      if (value == null) { result.Append("null"); return; }
      if (value is string text) { result.Append('\"').Append(text.Replace("\\", "\\\\").Replace("\"", "\\\"")).Append('\"'); return; }
    ...
    private readonly struct UpdateScope : IDisposable {
      ...
      public UpdateScope(FormController owner) => _owner = owner;
      public void Dispose() => _owner.EndUpdate();
    }
  }
}
