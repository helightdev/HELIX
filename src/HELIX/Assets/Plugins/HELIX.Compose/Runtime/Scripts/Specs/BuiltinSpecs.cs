...
using TextStyle = HELIX.Compose.TextStyle;
...
namespace HELIX.Compose {
  public readonly struct ChevronSpec : ISpec<ChevronSpec> {
    ...
    public ReadComposable<ChevronSpec> GetDefault(in ChevronSpec spec) {
      return Default;
    }
  ...
  public readonly struct IconRef : ISpec<IconRef> {
    ...
    public ReadComposable<IconRef> GetDefault(in IconRef spec) {
      return Default;
    }

    public static void Default(ref Composition cx, in IconRef spec) {
      if (spec.font != null) cx.Text(spec.icon).TextFont(spec.font);
      else cx.Text(spec.icon).TextFont(new StyleFontDefinition(spec.fontAsset));
    }
  }
  ...
  public readonly struct LabelSpec : ISpec<LabelSpec> {
    ...
    public ReadComposable<LabelSpec> GetDefault(in LabelSpec spec) {
      return Default;
    }
