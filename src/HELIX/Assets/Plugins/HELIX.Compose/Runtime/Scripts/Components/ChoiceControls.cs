...
namespace HELIX.Compose {
  public readonly struct SegmentedChoiceStyle {
    ...
    public SegmentedChoiceStyle(
      HXControlBoxStyle left,
      HXControlBoxStyle middle,
      HXControlBoxStyle right,
  ...
  public readonly struct SpinboxChoiceStyle {
    ...
    public SpinboxChoiceStyle(
      HXControlBoxStyle value,
      HXControlBoxStyle left,
      HXControlBoxStyle right,
      HXControlBoxStyle up,
      HXControlBoxStyle down,
      bool wrap = true,
      bool vertical = false,
      float gap = 0f
    ) {
  ...
  internal partial class SegmentedChoiceItem {
    ...
    private void OnCompose(ref Composition cx) {
      ...
      style.RenderBoundary(
        ref cx,
        InputState | (props.owner?.State ?? State.None) |
  ...
  public sealed partial class SegmentedChoice<T> : IChoiceItemOwner {
    ...
    bool IChoiceItemOwner.IsSelected(int index) {
      return _equality.Equals(props.value, _values[index]);
    }
  ...
  public sealed partial class ChoiceSpinbox<T> {
    ...
    private int SelectedIndex() {
      for (var i = 0; i < _values.Length; i++)
        if (_equality.Equals(props.value, _values[i]))
          return i;
      return -1;
    }
    ...
    private static void Previous(CompositionContext context) {
      context.Lookup<ChoiceSpinbox<T>>()?.Change(-1);
    }

    private static void Next(CompositionContext context) {
      context.Lookup<ChoiceSpinbox<T>>()?.Change(1);
    }
  ...
  public static class ChoiceControlExtensions {
    public static ref ElementRef SegmentedChoice<T>(
      ...
      bool enabled = true,
      bool error = false,
      SegmentedChoiceStyle? style = null
    ) {
