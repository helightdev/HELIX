...
namespace HELIX.Compose {
  public sealed class NavigationController : Signal<int> {
    ...
    public NavigationChange ActiveChange => IsTransitioning ? _change : null;
    ...
    public void SetGraph(NavigationGraph graph, bool preserveStack = false) {
      ...
      if (IsTransitioning) CompleteActive(false);

      Graph = graph;
      if (preserveStack && _backStack.Count > 0 && AreEntriesValidFor(graph)) RemapStack(graph);
      else {
    ...
    public bool Navigate(
      ...
      return Push(route, arguments, options);
    }
    ...
    public bool Push(
      ...
      return SubmitPush(route, arguments, options);
    }
    ...
    public NavigationSubmission SubmitPush(
      ...
      return SubmitRoute(NavigationOperationKind.Push, route, arguments, options);
    }
    ...
    public bool Push(
      ...
      return SubmitPush(page, arguments, options);
    }
    ...
    public NavigationSubmission SubmitPush(
      ...
      return SubmitPage(NavigationOperationKind.Push, page, arguments, options);
    }
    ...
    public bool Push(
      ...
      return Push(NavigationPage.Build(builder).Build(), arguments, options);
    }
    ...
    public bool Push(
      ...
      return Push(page?.Build(), arguments, options);
    }
    ...
    public Awaitable<T> Push<T>(
      ...
      return Push<T>(NavigationPage.Build(builder).Build(), arguments, options);
    }
    ...
    public NavigationSubmission Push<T>(
      ...
      return Push(NavigationPage.Build(builder).Build(), onResult, arguments, options, onError);
    }
    ...
    public Awaitable<T> Push<T>(
      ...
      return Push<T>(page?.Build(), arguments, options);
    }
    ...
    public NavigationSubmission Push<T>(
      ...
      return Push(page?.Build(), onResult, arguments, options, onError);
    }
    ...
    public bool Replace(
      ...
      return SubmitRoute(NavigationOperationKind.Replace, route, arguments, options);
    }
    ...
    public bool Replace(
      ...
      return SubmitPage(NavigationOperationKind.Replace, page, arguments, options);
    }
    ...
    public bool Replace(
      ...
      return Replace(NavigationPage.Build(builder).Build(), arguments, options);
    }
    ...
    public bool Replace(
      ...
      return Replace(page?.Build(), arguments, options);
    }
    ...
    public bool Go(
      ...
      return SubmitRoute(NavigationOperationKind.Go, route, arguments, options);
    }
    ...
    public bool Activate(
      ...
      return SubmitRoute(NavigationOperationKind.Activate, route, arguments, options);
    }
    ...
    public bool Activate(
      ...
      return SubmitPage(NavigationOperationKind.Activate, page, arguments, options);
    }
    ...
    public bool Activate(
      ...
      if (entry == null) return Reject(id, NavigationFailure.TargetNotFound, null, "The navigation entry is null.");
      return Enqueue(
        NavigationRequest.ToEntry(
          id,
    ...
    public bool Preload(string route, NavigationArguments arguments = null) {
      return SubmitRoute(NavigationOperationKind.Preload, route, arguments, NavigationOptions.Default);
    }
    ...
    public bool Preload(NavigationPage page, NavigationArguments arguments = null) {
      return SubmitPage(NavigationOperationKind.Preload, page, arguments, NavigationOptions.Default);
    }
    ...
    public bool Pop<T>(T result) {
      return SubmitPop(result, true);
    }

    public NavigationSubmission SubmitPop(object result = null, bool hasResult = false) {
      return Enqueue(
    ...
    public bool PopTo(string route, bool inclusive = false) {
      return SubmitPopTo(route, inclusive);
    }

    public NavigationSubmission SubmitPopTo(string route, bool inclusive = false) {
      return Enqueue(
    ...
    internal void DetachPresenter(object presenter) {
      ...
      _presenter = null;
      if (IsTransitioning) CompleteActive();
    }

    internal bool TryStartPresentation(long changeId) {
      if (!IsTransitioning || _change.Id != changeId || _change.Started) return false;
      _change.Started = true;
      var exception = Dispatch(_change.Entering, true, false);
      exception = Dispatch(_change.Exiting, false, false, exception);
      if (exception != null) throw exception;
      return true;
    }

    internal void CompletePresentation(long changeId) {
      if (!IsTransitioning || _change.Id != changeId) return;
      CompleteActive();
    }
    ...
    internal void FailPresentation(NavigationEntry entry, Exception exception) {
      ...
      if (!IsTransitioning || !_change.EnteringMutable.Contains(entry)) return;
    ...
    internal NavigationPresentation PresentationOf(NavigationEntry entry, bool covered) {
      ...
      if (IsTransitioning && _change.EnteringMutable.Contains(entry)) value |= NavigationPresentation.Entering;
      if (IsTransitioning && _change.ExitingMutable.Contains(entry)) value |= NavigationPresentation.Exiting;
      return value;
    }
    ...
    public override void Dispose() {
      ...
      if (IsTransitioning) FailEntries(_change.Removed, new ObjectDisposedException(nameof(NavigationController)));
      _backStack.Clear();
      ClearCachedEntries();
      _presentation.Clear();
      IsTransitioning = false;
    ...
    private void Pump() {
      while (!IsTransitioning && _requests.Count > 0 && !IsDisposed) {
        ...
        case NavigationOperationKind.Push:
          ...
          if (request.Options.Has(NavigationBehavior.ClearStack)) _backStack.Clear();
            ...
            var keepCount = request.Options.Has(NavigationBehavior.PopUpToInclusive)
              ? popIndex
              : popIndex + 1;
            if (keepCount < _backStack.Count) _backStack.RemoveRange(keepCount, _backStack.Count - keepCount);
          }
    ...
    private void PrepareChange(in NavigationRequest request, NavigationPage targetPage) {
      ...
      if (request.Operation is NavigationOperationKind.Pop or NavigationOperationKind.PopTo &&
        _backStack.Count > 0 && _before.Count > 0 &&
        _before[^1].Opaque) AddUnique(_change.EnteringMutable, _backStack[^1]);

      _popResult = request.Result;
      _hasPopResult = request.HasResult;
      IsTransitioning = _change.Added.Count > 0 || _change.Removed.Count > 0;
      if (!IsTransitioning) SynchronizePresentation();
    }

    private void CompleteActive(bool pump = true) {
      if (!IsTransitioning) return;
      var exception = Dispatch(_change.Entering, true, true);
      exception = Dispatch(_change.Exiting, false, true, exception);
    ...
    private Exception Dispatch(
      ...
      var phase = entering
        ...
        : completed
          ? NavigationLifecyclePhase.Exited
          : NavigationLifecyclePhase.Exiting;
    ...
    private int FindEntryIndex(string route) {
      for (var i = _backStack.Count - 1; i >= 0; i--)
        if (string.Equals(_backStack[i].Name, route, StringComparison.Ordinal))
          return i;
      return -1;
    }
    ...
    private NavigationEntry FindExisting(NavigationRoute route, NavigationPage page) {
      for (var i = _backStack.Count - 1; i >= 0; i--)
        if (Matches(_backStack[i], route, page))
          return _backStack[i];
      for (var i = _cached.Count - 1; i >= 0; i--)
        if (Matches(_cached[i], route, page))
          return _cached[i];
      return null;
    }
    ...
    private NavigationEntry CreateEntry(
      ...
      return new NavigationEntry(_nextEntryId++, route, page, arguments ?? NavigationArguments.Empty, completion);
    }
    ...
    private long ClaimOperationId() {
      return _nextOperationId++;
    }

    private IBoundary ResultBoundary() {
      return Current?.Boundary ?? throw new InvalidOperationException(
        "A composable navigation result callback requires a currently presented navigation page."
      );
    }

    private bool Owns(NavigationRoute route) {
      return Graph?.Contains(route) == true;
    }

    private static bool Matches(NavigationEntry entry, NavigationRoute route, NavigationPage page) {
      return entry != null && (route != null
    ...
    private static string FailureMessage(NavigationFailure failure) {
      return failure switch {
        ...
        NavigationFailure.ResultUnavailable =>
          "A single-top operation cannot create a second result for an existing entry.",
  ...
  internal readonly struct NavigationRequest {
    ...
    internal static NavigationRequest ToPage(
      ...
      return new NavigationRequest(id, operation, null, page, null, arguments, options, completion, null, false, false);
    }
    ...
    internal static NavigationRequest ToEntry(
      ...
      return new NavigationRequest(id, operation, null, null, entry, arguments, options, null, null, false, false);
    }

    internal static NavigationRequest Pop(long id, object result, bool hasResult) {
      return new NavigationRequest(
        id,
        NavigationOperationKind.Pop,
        null,
        null,
        null,
        null,
    ...
    internal static NavigationRequest PopTo(long id, string route, bool inclusive) {
      return new NavigationRequest(
        id,
        NavigationOperationKind.PopTo,
        route,
        null,
        null,
        null,
