...
namespace HELIX.Compose {
  ...
  internal sealed partial class OverlayContentBoundary {
    ...
    private void OnCompose(ref Composition cx) {
      ...
      using (cx.WriteContext(out var context)) OverlayContextData.Key[in context] = contextData;
  ...
  internal sealed class OverlayEntryElement : VisualElement, IDisposable {
    ...
    public bool ApplyPlacement(float stackOffset) {
      ...
      } else ResolveViewport(options.placement, hostRect, width, height, margin, out x, out y);
    ...
    private float ResolveAnchorWidth() {
      return TryResolveAnchorRect(out var anchor) ? anchor.width : float.NaN;
    }
    ...
    private bool IsInsideOverlayFamily(VisualElement element) {
      return IsInsideOverlayFamily(element, _entry);
    }
