...
namespace HELIX.Compose {
  ...
  public sealed class DropdownController<T> : DropdownController {
    ...
    public override string Label(int index) {
      return options[index].label ?? string.Empty;
    }

    public override bool IsEnabled(int index) {
      return options[index].enabled;
    }

    public override bool IsSelected(int index) {
      return _equality.Equals(options[index].value, value);
    }
  ...
  internal partial class PopupMenuItemBoundary {
    ...
    private void OnCompose(ref Composition cx) {
      ...
      using (cx.Group(Axis.Horizontal, Justify.SpaceBetween, Align.Center)) {
        ...
        if (props.hasChildren) {
          ...
          new ChevronSpec(
            ArrowPosition.Right,
            ThemeProperties.ChevronSize[in cx],
            TextStyle.Key[in cx].color
          ).Compose(ref cx);
        }
      }
    }
  ...
  public partial class HXDropdownButton : IPopupMenuItemOwner {
    ...
    private void OnCompose(ref Composition cx) {
      ...
      using (cx.Group(Axis.Horizontal, Justify.SpaceBetween, Align.Center)) {
        ...
        new ChevronSpec(
          _menu?.IsOpen == true ? ArrowPosition.Up : ArrowPosition.Down,
          ThemeProperties.ChevronSize[in cx],
    ...
    private void EnsureController(DropdownController given) {
      if (given == null) {
        ...
        if (factory == null) throw new InvalidOperationException("An implicit dropdown requires a controller factory.");
  ...
  public partial class HXMenuButton {
    ...
    private void OnCompose(ref Composition cx) {
      ...
      using (cx.Group(Axis.Horizontal, Justify.SpaceBetween, Align.Center)) {
        ...
        cx.Spacing(1);
        new ChevronSpec(
          _presenter?.IsOpen == true ? ArrowPosition.Up : ArrowPosition.Down,
          ThemeProperties.ChevronSize[in cx],
          _resolvedStyle.iconColor
        ).Compose(ref cx);
      }
            ...
            case MenuItemKind.Separator:
              cx.DrawSolidBox(
                ...
                constraints: BoxConstraints.Preferred(
                  StyleKeyword.Auto,
                  Mathf.Max(0f, _style.separatorThickness)
                )
              );
              break;
  ...
  public static class ManualDropdownButtonExtensions {
    ...
    public static ref ElementRef DropdownButton<T>(
      ...
      return ref HXDropdownButton.ComposeBoundary(
        ref cx,
        null,
        style,
        value,
    ...
    private static class AutomaticController<T> {
      ...
      private static DropdownController Create() {
        return new DropdownController<T>();
      }
    }
  }
}
