...
namespace HELIX.Editor {
  public static class StyleLengthSerializer {
    ...
    public static StyleLength FromString(string str) {
      ...
      switch (str) {
        case "null": return new StyleLength(StyleKeyword.Null);
        case "auto": return new StyleLength(StyleKeyword.Auto);
        case "none": return new StyleLength(StyleKeyword.None);
        case "initial": return new StyleLength(StyleKeyword.Initial);
      }

      if (str.Equals("0")) return new StyleLength(new Length(0, LengthUnit.Pixel));
  ...
  public class StyleLength2Converter : UxmlAttributeConverter<StyleLength2> {
    public override StyleLength2 FromString(string value) {
      ...
      if (parts.Length != 2) throw new FormatException("Invalid StyleLength2 format. Expected format: 'width,height'");
  ...
  public class StyleLength4Converter : UxmlAttributeConverter<StyleLength4> {
    public override StyleLength4 FromString(string value) {
      var parts = value.Split(',');
      if (parts.Length != 4)
        throw new FormatException("Invalid StyleLength4 format. Expected format: 'left,top,right,bottom'");
