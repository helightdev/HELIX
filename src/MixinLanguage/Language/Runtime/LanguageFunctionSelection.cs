...
internal sealed partial class LanguageExecution {
  private readonly record struct FunctionMatch(BytecodeFunction Function, BytecodeSignature Signature,
    IMixinValue Parameter, LanguageFunctionScope Owner);
  ...
  private IMixinValue SelectFunction(string name, IMixinValue[] arguments, LanguageFunctionScope binding,
    ...
    IMixinValue[] bestArguments = null;
    IMixinValue bestParameter = null;
    ...
    foreach (var candidate in candidates) {
      ...
      IMixinValue suppliedParameter = null;
      if (signature is {Inputs: null}) {
        ...
        if (arguments.Length == 1 && arguments[0] is MixinTableValue supplied && !candidate.Variadic) {
          ...
          KeyValuePair<MixinString, IMixinValue>[] convertedEntries = null;
          namedMatch = true;
          ...
          if (namedMatch) suppliedParameter = convertedEntries == null ? supplied : new MixinTableValue(convertedEntries);
        }
        if (!namedMatch) {
          ...
          for (var i = 0; i < arguments.Length; i++) {
            ...
            if (count != 0) {
              if (ReferenceEquals(convertedArguments, arguments)) convertedArguments = (IMixinValue[])arguments.Clone();
    ...
    if (bestParameter == null) {
      if (best.Signature == null) bestParameter = Pack(arguments);
        ...
        var entries = new KeyValuePair<MixinString, IMixinValue>[fields.Count];
        for (var i = 0; i < fields.Count; i++) {
          IMixinValue value;
          if (fields[i].Variadic) {
            var tail = new IMixinValue[bestArguments.Length - i];
            Array.Copy(bestArguments, i, tail, 0, tail.Length);
            value = new TupleMixinValue(tail);
          } else value = bestArguments[i];
          entries[i] = new(context.ResolveString(fields[i].Name), value);
        }
        bestParameter = new MixinTableValue(entries);
      }
    }
