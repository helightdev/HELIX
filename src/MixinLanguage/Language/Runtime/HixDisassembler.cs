...
internal static class HixDisassembler {
  internal static string Render(MixinExpressionExecutionProgram program, IReadOnlyList<byte> code,
    ...
    if (includePools) text.Append(MixinExpressionExecutionProgram.DisassemblePools(strings, constants)).AppendLine();
  ...
  private static (string Operands, string Pseudocode) Describe(HixInstruction instruction, int address,
    MixinStringPool strings, IReadOnlyList<IMixinValue> constants) {
  ...
  private static string Constant(IMixinValue value) => value switch {
    NumberMixinValue number => number.Value.ToString("R", CultureInfo.InvariantCulture),
    BooleanMixinValue boolean => boolean.Value ? "true" : "false",
    NullMixinValue => "null",
    KindMixinValue kind => kind.Name,
    _ => value.ToString()
  };
