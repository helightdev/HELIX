...
internal sealed partial class LanguageExecution {
  ...
  private readonly struct ExecutionFrame {
    ...
    private readonly List<IMixinValue> selectors;
    private readonly Stack<(int Target, int Stack, int Selectors, IMixinValue Selector)> checks;
    private readonly IMixinValue initialSelector;
    ...
    internal ExecutionFrame(LanguageExecution execution, int address) {
      ...
      selectors = new List<IMixinValue>();
      checks = new Stack<(int, int, int, IMixinValue)>();
      initialSelector = execution.selector;
    }
    ...
    private void Reset() {
      stack.Clear(); selectors.Clear(); checks.Clear();
      execution.selector = initialSelector;
    }
      ...
      try {
        for (var pc = address; pc < end;) {
          ...
          switch (instruction.Opcode) {
            ...
            case HixOpcode.Check: checks.Push((instructionAddress + a, stack.Count, selectors.Count, execution.selector)); break;
            ...
            case HixOpcode.PushSelector: selectors.Add(execution.selector); execution.selector = MixinStorageValue.Capture(Pop()); break;
            case HixOpcode.PopSelector:
              execution.selector = selectors[selectors.Count - 1]; selectors.RemoveAt(selectors.Count - 1); break;
            case HixOpcode.MatchSelector: stack.Add(Bool(execution.Equal(execution.selector, Pop()))); break;
          ...
          if (completion.Kind == BytecodeFlow.Error && checks.Count != 0) {
            ...
            execution.selector = handler.Selector;
        ...
        return default;
      } finally { execution.selector = initialSelector; }
    }
  }
}
