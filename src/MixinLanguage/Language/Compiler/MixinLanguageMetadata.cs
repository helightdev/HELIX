...
public sealed record MixinDirectiveDescriptor(
  ...
  MixinArgumentRole OperandRole, IReadOnlyList<MixinArgumentRole> ArgumentRoles,
  string Documentation, DirectiveDefinition Definition
) {
  public MixinDirectiveArgumentMetadata GetArgumentMetadata(int argumentCount, int index) =>
    Definition.GetArgumentMetadata(argumentCount, index);
}

public sealed record MixinFunctionDescriptor(
  string Name, int MinimumArguments, int MaximumArguments, bool IsPredicate,
  MixinReceiverKind ReceiverKind, MixinReceiverKind ResultKind,
  IReadOnlyList<MixinArgumentRole> ArgumentRoles,
  string Documentation
);

public sealed record MixinRootDescriptor(
  string Name, MixinExpressionRoot Root, string Documentation, MixinRootDefinition Definition
);
...
public static class MixinLanguageCatalog {
  public static IReadOnlyList<MixinDirectiveDescriptor> Directives { get; } = [
    .. DirectiveLibrary.EnumerateLanguageDefinitions().Select(value => new MixinDirectiveDescriptor(
      ...
      Convert(value.OperandType), value.ArgumentTypes.Select(Convert).ToArray(), value.Documentation, value
    ))
  ];
  public static IReadOnlyList<MixinFunctionDescriptor> Functions { get; } = [
    .. FunctionLibrary.Enumerate().Select(value => new MixinFunctionDescriptor(
      ...
      Receiver(value.ReceiverType), Receiver(value.ResultType),
      value.ArgumentTypes.Select(Convert).ToArray(), value.Documentation
    ))
  ];
  public static IReadOnlyList<MixinRootDescriptor> Roots { get; } = [
    .. MixinRootLibrary.Enumerate().Select(value => new MixinRootDescriptor(
      value.Name, value.Root, value.Documentation, value
    ))
  ];
