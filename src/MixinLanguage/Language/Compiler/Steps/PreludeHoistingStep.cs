...
public sealed class PreludeHoistingStep : MixinExpressionCompilerStep {
  ...
  private static bool TryHoistRoslynReferences(
    ...
    foreach (var parsed in expression.AvailableInstructions()) {
      if (parsed is CallAst call && !localFunctions.Contains(call.Function ?? "")) {
      ...
      if (parsed is DirectiveInvocationAst { Definition.HoistedLocalArgumentIndex: >= 0 }) {
  ...
  internal static void HoistLateCarries(
    ...
    var carries = lateExpression.AvailableInstructions().OfType<CarryAst>()
      .Cast<InstructionAst>().ToArray();
    ...
    late = new ProgramAst(
      lateExpression.AvailableInstructions()
        .Where(item => item is not CarryAst)
    );
  }
}
