...
public sealed class MixinProgramSyntax {
  ...
  internal void CollectConstants(MixinStringPoolBuilder pool) { foreach (var item in _instructions) { pool.Intern(item.Command); item.CollectConstants(pool); } }
}
...
public abstract class ValueDirectiveSyntax(int line, IReadOnlyList<ValueExpressionPart> expression) : DirectiveInstruction(line) {
...
public abstract class BooleanDirectiveSyntax(int line, IReadOnlyList<MixinExpressionReference> expression) : DirectiveInstruction(line) {
  internal IReadOnlyList<MixinExpressionReference> Expression { get; } = expression ?? [];
  internal override void CollectConstants(MixinStringPoolBuilder pool) { foreach (var item in Expression) item.CollectConstants(pool); }
}
...
public sealed class UnknownDirectiveSyntax(int line, string command) : DirectiveInstruction(line) {
  ...
  internal override void CollectConstants(MixinStringPoolBuilder pool) => pool.Intern(Name);
}
public sealed class DirectiveInvocationSyntax(int line, DirectiveDefinition definition, IReadOnlyList<DirectiveArgumentSyntax> arguments, IReadOnlyList<ValueExpressionPart> operand) : ValueDirectiveSyntax(line, operand) {
  ...
  internal new IReadOnlyList<string> Arguments { get; } = [.. arguments.Select(MixinSyntaxRenderer.RenderArgument)];
...
public sealed class CallDirectiveSyntax(int l, string function, string returnLocal, IReadOnlyList<ValueExpressionPart> parameter) : ValueDirectiveSyntax(l,parameter) { internal string Function { get; } = function; internal string ReturnLocal { get; } = returnLocal; }
...
public sealed class MatchDirectiveSyntax(int l, string failureLabel, IReadOnlyList<MixinExpressionReference> condition) : BooleanDirectiveSyntax(l,condition) { internal string FailureLabel { get; } = failureLabel; }
public sealed class AssertDirectiveSyntax(int l, IReadOnlyList<MixinExpressionReference> condition) : BooleanDirectiveSyntax(l,condition);
public sealed class CodeDirectiveSyntax(int l, MixinExpressionOutputTarget target, string injectionTarget, IReadOnlyList<ValueExpressionPart> code) : ValueDirectiveSyntax(l,code) { internal MixinExpressionOutputTarget Target { get; } = target; internal string InjectionTarget { get; } = injectionTarget; }
public sealed class MixinDirectiveSyntax(int l, DirectiveArgumentSyntax target, DirectiveArgumentSyntax priority, IReadOnlyList<ValueExpressionPart> code) : ValueDirectiveSyntax(l,code) { internal DirectiveArgumentSyntax Target { get; } = target; internal DirectiveArgumentSyntax Priority { get; } = priority; }
public sealed class UsingDirectiveSyntax(int l, IReadOnlyList<ValueExpressionPart> value) : ValueDirectiveSyntax(l,value);
public sealed class LogDirectiveSyntax(int l, IReadOnlyList<ValueExpressionPart> message) : ValueDirectiveSyntax(l,message);
public sealed class LocalDirectiveSyntax(int l, string name, IReadOnlyList<ValueExpressionPart> value) : ValueDirectiveSyntax(l,value) { internal string Name { get; } = name; }
public sealed class VariableDirectiveSyntax(int l, string name, IReadOnlyList<ValueExpressionPart> value) : ValueDirectiveSyntax(l,value) { internal string Name { get; } = name; }
public sealed class CarryDirectiveSyntax(int l, string label, IReadOnlyList<ValueExpressionPart> value) : ValueDirectiveSyntax(l,value) { internal string Label { get; } = label; }
public sealed class ReturnDirectiveSyntax(int l, IReadOnlyList<ValueExpressionPart> value) : ValueDirectiveSyntax(l,value);
...
public sealed class FailDirectiveSyntax(int l, IReadOnlyList<ValueExpressionPart> message) : ValueDirectiveSyntax(l,message);
...
public sealed record ValueExpressionPart(string Literal, MixinExpressionReference Reference);
...
internal static class MixinSyntaxFacts {
  internal static string Command(DirectiveInstruction n) => n switch {
    EmptyDirectiveSyntax => null, UnknownDirectiveSyntax x => x.Name, DirectiveInvocationSyntax x => x.Definition.Name,
  ...
  internal static IReadOnlyList<string> Arguments(DirectiveInstruction n) => n switch {
    ...
    CodeDirectiveSyntax { Target: not MixinExpressionOutputTarget.Target } x => [x.Target.ToString().ToUpperInvariant()],
    MixinDirectiveSyntax { Priority: not null } x => [MixinSyntaxRenderer.RenderArgument(x.Target), MixinSyntaxRenderer.RenderArgument(x.Priority)], MixinDirectiveSyntax x => [MixinSyntaxRenderer.RenderArgument(x.Target)],
  ...
  internal static string Operand(DirectiveInstruction n) => n switch {
    ValueDirectiveSyntax x => MixinSyntaxRenderer.RenderValue(x.Expression), BooleanDirectiveSyntax x => MixinSyntaxRenderer.RenderBoolean(x.Expression), _ => ""
  };
}
