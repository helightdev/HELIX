...
internal sealed class MixinValueDictionary : IDictionary<string, object>, IReadOnlyDictionary<string, object> {
  ...
  internal MixinValueDictionary() { _values = new Dictionary<string, IMixinValue>(StringComparer.Ordinal); }
  internal MixinValueDictionary(IEnumerable<KeyValuePair<string, object>> values) : this() {
    foreach (var item in values) this[item.Key] = item.Value;
  }

  public object this[string key] {
    get => _values[key].BackingValue;
    set => _values[key] = Close(MixinValue.From(value));
  }
  ...
  public void Add(string key, object value) => _values.Add(key, Close(MixinValue.From(value)));
  public void Add(KeyValuePair<string, object> item) => Add(item.Key, item.Value);
  public bool ContainsKey(string key) => _values.ContainsKey(key);
  public bool Remove(string key) => _values.Remove(key);
  public bool TryGetValue(string key, out object value) {
    if (_values.TryGetValue(key, out var typed)) { value = typed.BackingValue; return true; }
  ...
  public IEnumerator<KeyValuePair<string, object>> GetEnumerator() => _values
    .Select(item => new KeyValuePair<string, object>(item.Key, item.Value.BackingValue)).GetEnumerator();
