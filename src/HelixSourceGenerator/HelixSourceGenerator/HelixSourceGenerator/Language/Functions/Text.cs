...
internal sealed class FloatTimeFunction : FunctionDefinition {
  ...
  private static bool TryParseFloatTime(string value, out float seconds) {
    ...
    if (text[0] == '%') {
      if (!double.TryParse(text.Substring(1).Trim(), NumberStyles.Float,
        CultureInfo.InvariantCulture, out var frequency) || frequency <= 0d) return false;
    ...
    if (!match.Success || !double.TryParse(match.Groups["number"].Value, NumberStyles.Float,
  ...
  private static string Format(float seconds) => Math.Abs(seconds) <= ZeroTolerance
    ? "-0f"
